...
function getFormatedDateWithMonthName(date) {
    ...
    var month = date.getMonth() + 1;
    var year = date.getFullYear();
    return getMonthNameInShort(month) + '/' + day + '/' + year;
...
function createDateForServer(value) {
...
function getFormData(className) {
    ...
    $('.' + className).each(function () {
        ...
        if (name != 'undifined') {
            if ($(this).is('input[type="text"]') || $(this).is('input[type="hidden"]') || $(this).is('input[type="password"]') || $(this).is('select') || $(this).is('textarea')) {
                //console.log(name + " val-" + $(this).val());
                if ($(this).val()) {
 ...
 function requestWithFile(url, type, data, stringfy, async, cache, successCallBack) {
    type == null ? 'GET' : 'POST';
...
function getDataWithFile(className) {
    ...
    $('.' + className).each(function () {
        ...
        if (name != 'undefined') {
            ...
            if (element.is('input[type="radio"]')) {
                if ($("input:radio." + name + ":checked")) {
                    formData.append(name, $("input:radio." + name + ":checked").val());
                } else {
...
function setFormData(data, exclues) {
    ...
    for (var key in data) {
        if (exclues.indexOf(key) == -1) {
            if ($('#' + key).is('input[type="text"]') || $('#' + key).is('input[type="hidden"]') || $('#' + key).is('input[type="password"]') || $('#' + key).is('select') || $('#' + key).is('textarea')) {
                $('#' + key).val(data[key]);
            }
...
function clearFormField(excludes) {
    if (excludes) {
        $('input[type=text],input[type=hidden],input[type=password],select,textarea').each(function () {
